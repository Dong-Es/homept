...
function main(){
    setMonthTitle(year,mon);
    makeCalendar(year,mon,getDayOfMon(mon,year));
    todoTitle.textContent = `이 날 ${year}.${mon}.${currentDay} 무엇을 하실껀가요?⁉`;
    displayToDoOnDays();
}
...
Day.addEventListener('click',(event)=>{
    ...
    if(event.target.className!=='disabled'){
        clearEvent();
        todoTitle.textContent = `이 날 ${year}.${mon}.${event.target.textContent} 무엇을 하실껀가요⁉`;
...
reset.addEventListener('click',()=>{
    const result = prompt(`이 ${year} ${mon} ${DayOfChoice}날 할일을 지우시겠습니까?  Y를 눌러주세요`);
    const YMD = year+'-'+mon+'-'+DayOfChoice;
    if(result==='y'){
...
allReset.addEventListener('click',()=>{
    const result = prompt(`정말 전부를 지우시겠습니까? 지우실꺼면 Y를 눌러주세요.`);
    if(result==='y'){
        localStorage.clear();
        displayToDoOnDays();
    }
